...
```{r}
ggplot(allsamples, aes(Variety, shootN + bgN, colour = Herbicide)) +
  geom_point() +
  geom_boxplot() +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "white"),
...
Clearly herbicide application resulted in less nitrogen in Herald plants, but probably not in FEH-1 plants.

But is this just because biomass was lower?  Let's take a look.

```{r}
ggplot(allsamples, aes(Variety, shootDM + bgDM, colour = Herbicide)) +
  geom_point() +
  geom_boxplot() +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "white"),
...
Hmm.  That's interesting.  Looks like there's a drop in biomass for FEH-1, the herbicide tolerant variety.  We'll need to take a closer look at that.  What if we restrict our analysis to inoculated plants only?

```{r}

ggplot(inoculated, aes(Variety, shootN + bgN, colour = Herbicide)) +
  geom_point() +
  geom_boxplot() +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "white"),
...
ggplot(inoculated, aes(Variety, shootDM + bgDM, colour = Herbicide)) +
  geom_point() +
  geom_boxplot() +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "white"),
...
#### ARA
Let's zero in on nitrogen fixation itself.  We have a measure on nitrogenase activity at the time of harvest - the acetylene reduction assay.
...
Is this purely a plant growth issue? Don't forget about the controls...
...
That's interesting, isn't it.  Obviously this isn;t purely a nitrogen fixation issue.  Plant growth is being reduced by chlorsulfuron even when plants are supplied with N.  
